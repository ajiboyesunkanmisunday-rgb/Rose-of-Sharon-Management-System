...
export async function uploadMedia(fields: {
  ...
  try {
    // Absolute URL — goes directly to api.rccgros.org, bypassing Netlify entirely.
...
export async function uploadLargeMedia(fields: {
  ...
  return new Promise<MediaResponse>((resolve, reject) => {
    ...
    // Absolute URL — goes directly to api.rccgros.org, bypassing Netlify.
...
export interface VotingCycle {
  ...
  month: number;
  year: number;
  status: "DRAFT" | "NOMINEES_PENDING" | "NOMINEES_APPROVED" | "VOTING_OPEN" | "VOTING_CLOSED" | "WINNER_ANNOUNCED";
...
export interface CreateVotingCategoryRequest {
  name: string;
  description?: string;
}
...
export async function getVotingCategories(): Promise<VotingCategory[]> {
  return apiFetch<VotingCategory[]>("/api/v1/voting/categories");
}

export async function createVotingCategory(body: CreateVotingCategoryRequest): Promise<VotingCategory> {
  return apiFetch<VotingCategory>("/api/v1/voting/categories", { method: "POST", body: JSON.stringify(body) });
}

export async function deleteVotingCategory(id: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/categories/${id}`, { method: "DELETE" });
}

export async function getVotingCycles(pageNo = 0, pageSize = 10, categoryId?: string, status?: string): Promise<CustomPageResponse<VotingCycle>> {
  const params = new URLSearchParams({ pageNo: String(pageNo), pageSize: String(pageSize) });
  if (categoryId) params.set("categoryId", categoryId);
  if (status) params.set("status", status);
  return apiFetch<CustomPageResponse<VotingCycle>>(`/api/v1/voting/cycles?${params}`);
}

export async function getVotingCycle(id: string): Promise<VotingCycle> {
  return apiFetch<VotingCycle>(`/api/v1/voting/cycles/${id}`);
}

export async function createVotingCycle(body: CreateVotingCycleRequest): Promise<VotingCycle> {
  return apiFetch<VotingCycle>("/api/v1/voting/cycles", { method: "POST", body: JSON.stringify(body) });
}

export async function generateNominees(cycleId: string): Promise<VotingNominee[]> {
  return apiFetch<VotingNominee[]>(`/api/v1/voting/cycles/${cycleId}/generate-nominees`, { method: "POST" });
}

export async function approveNominees(cycleId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/approve-nominees`, { method: "POST" });
}

export async function swapNominee(cycleId: string, nomineeId: string, newUserId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/nominees/${nomineeId}/swap`, { method: "PATCH", body: JSON.stringify({ newUserId }) });
}

export async function scheduleVoting(cycleId: string, startDate: string, endDate: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/schedule`, { method: "PATCH", body: JSON.stringify({ votingStartDate: startDate, votingEndDate: endDate }) });
}

export async function extendVotingDeadline(cycleId: string, newEndDate: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/extend`, { method: "PATCH", body: JSON.stringify({ votingEndDate: newEndDate }) });
}

export async function openVoting(cycleId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/open`, { method: "PATCH" });
}

export async function closeVoting(cycleId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/close`, { method: "PATCH" });
}

export async function announceWinner(cycleId: string, nomineeId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/announce-winner`, { method: "PATCH", body: JSON.stringify({ nomineeId }) });
}

export async function cancelVotingCycle(cycleId: string): Promise<OperationalResponse> {
  return apiFetch<OperationalResponse>(`/api/v1/voting/cycles/${cycleId}/cancel`, { method: "PATCH" });
}

export async function getVotingResults(cycleId: string): Promise<VotingResultsResponse> {
  return apiFetch<VotingResultsResponse>(`/api/v1/voting/cycles/${cycleId}/results`);
}

export async function getHallOfFame(pageNo = 0, pageSize = 20): Promise<CustomPageResponse<VotingCycle>> {
  return apiFetch<CustomPageResponse<VotingCycle>>(`/api/v1/voting/hall-of-fame?pageNo=${pageNo}&pageSize=${pageSize}`);
}
