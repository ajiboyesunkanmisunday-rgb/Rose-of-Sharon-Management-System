...
export default function EditSecondTimerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : "Failed to update second timer.");
      setSubmitting(false);
    }
  };
    ...
    <DashboardLayout>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-8 rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-2">
            <div>
              ...
              <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} onBlur={() => touch("firstName")} placeholder="Enter First Name" className={`${inputStyles} ${touched.firstName && fieldErrors.firstName ? "border-red-400" : ""}`} />
            ...
            <div>
              ...
              <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} onBlur={() => touch("lastName")} placeholder="Enter Last Name" className={`${inputStyles} ${touched.lastName && fieldErrors.lastName ? "border-red-400" : ""}`} />
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" variant="primary" disabled={submitting || !isFormValid}>{submitting ? "Saving…" : "Save Changes"}</Button>
        </div>
      </form>
