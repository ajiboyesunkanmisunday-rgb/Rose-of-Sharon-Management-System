...
export default function SecondTimersPage() {
  ...
  const fetchTimers = useCallback(async (page: number) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to load second timers.";
      setApiError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    displayedTimers.forEach((st) => (checked ? next.add(st.id) : next.delete(st.id)));
  ...
  const allPageSelected =
    displayedTimers.length > 0 && displayedTimers.every((st) => selectedRows.has(st.id));
  ...
  const handleExport = () => {
    const csv = toCSV(
      displayedTimers.map((st) => ({
        ...
        assignedFollowUp: st.assignedFollowUp ? fullName(st.assignedFollowUp) : "",
        calls: st.noOfCalls ?? 0,
        visits: st.noOfVisits ?? 0,
      }))
    );
    downloadCSV(csv, `second-timers-export-${new Date().toISOString().slice(0, 10)}.csv`);
  };
  ...
  return (
    <DashboardLayout>
      ...
      <BulkImportModal
        ...
        onImport={(rows) => {
          console.log("Bulk import Second Timers:", rows);
