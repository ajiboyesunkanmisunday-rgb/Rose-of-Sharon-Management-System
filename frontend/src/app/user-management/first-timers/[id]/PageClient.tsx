...
import { getUser, addCallReport, addVisitReport, getNotes, deleteNote, convertToSecondTimer, type UserResponse, type NoteResponse } from "@/lib/api";
...
export default function ViewFirstTimerPage() {
  ...
  const fetchNotes = useCallback(async () => {
    ...
    try {
      ...
      setNotes(data);
    } catch { /* non-fatal — silently ignore if endpoint isn't ready */ }
    ...
    <DashboardLayout>
      <div className="mb-4">
        ...
        <div className="flex items-center gap-2 mt-1">
          <button onClick={() => router.push("/user-management/first-timers")} className="flex items-center text-[#000080] dark:text-indigo-400 transition-colors hover:text-[#000066]">
            <BackArrow />
          </button>
          <h2 className="text-[22px] font-bold text-[#000080] dark:text-indigo-400">View First Timer</h2>
        </div>
      </div>
