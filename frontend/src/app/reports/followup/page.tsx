    ...
    <DashboardLayout>
      ...
      <div className="mb-6">
        <button
          ...
          className="mb-4 flex items-center gap-1.5 text-sm font-medium text-[#000080] hover:text-[#000066]"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[28px] font-bold text-[#000000]">Urgent Followup List</h1>
          ...
          <button
            onClick={load}
            disabled={loading}
            className="ml-auto flex items-center gap-2 rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-xs font-medium text-[#374151] hover:border-[#000080] hover:text-[#000080] disabled:opacity-50"
          >
      ...
      {!loading && (
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 text-center">
            ...
            <p className="mt-1 text-xs text-[#6B7280]">Awaiting followup</p>
          </div>
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 text-center">
            <p className="text-2xl font-bold text-[#000080]">{urgentAll.filter((u) => u.userType?.toLowerCase().includes("first")).length}</p>
            <p className="mt-1 text-xs text-[#6B7280]">First timers</p>
          </div>
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 text-center">
            <p className="text-2xl font-bold text-[#7C3AED]">{urgentAll.filter((u) => u.userType?.toLowerCase().includes("second")).length}</p>
            <p className="mt-1 text-xs text-[#6B7280]">Second timers</p>
          </div>
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4 text-center">
            <p className="text-2xl font-bold text-[#D97706]">{urgentAll.filter((u) => daysSince(u.createdOn) > 30).length}</p>
            <p className="mt-1 text-xs text-[#6B7280]">Over 30 days old</p>
          </div>
        </div>
      )}
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded-lg border border-[#E5E7EB] bg-white px-3 py-2.5 text-sm text-[#374151] focus:border-[#000080] focus:outline-none"
        >
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#F9FAFB]">
              <tr className="border-b border-[#E5E7EB]">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-[#6B7280]">Assigned To</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-[#6B7280]">Date Added</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-[#6B7280]">Days Waiting</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F3F4F6]">
              {paginated.map((u, i) => {
                const days    = daysSince(u.createdOn);
                const urgency = days > 30 ? "text-red-600 font-bold" : days > 14 ? "text-amber-600 font-semibold" : "text-[#374151]";
                const assigned = u.assignedFollowUp ? fullName(u.assignedFollowUp) : null;

                return (
                  <tr key={u.id} className={`hover:bg-[#FAFAFA] ${days > 30 ? "bg-red-50/30" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-[#111827]">{fullName(u)}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-xs">
                      {assigned ? (
                        <div className="flex items-center gap-1.5">
                          <UserCheck className="h-3.5 w-3.5 text-[#000080]" />
                          <span className="font-medium text-[#000080]">{assigned}</span>
                        </div>
                      ) : (
                        <span className="text-[#9CA3AF]">Not assigned</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs text-[#374151]">{fmtDate(u.createdOn)}</td>
