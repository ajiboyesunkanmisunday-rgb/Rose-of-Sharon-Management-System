...
export default function GuestWorkflowTestPage() {
  ...
  const inp = "w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm text-[#111827] outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB]";
    ...
    <DashboardLayout>
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#6B7280]">Registers a first-timer as they would fill the public church form</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-5">
            <h2 className="mb-3 text-sm font-bold text-[#111827]">Profile Photo</h2>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[10px] text-[#9CA3AF]">Optional · JPG or PNG</p>
              </div>
            </div>
          ...
          {/* Visit Details */}
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-5">
            <h2 className="mb-4 text-sm font-bold text-[#111827]">Visit Details</h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {/* Toggles */}
              <div className="flex items-center justify-between rounded-lg border border-[#E5E7EB] px-3 py-2.5">
                <span className="text-sm text-[#374151]">Just visiting?</span>
                <button type="button" onClick={() => setIsVisiting(!isVisiting)}
                  ...
                  <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${isVisiting ? "translate-x-4" : "translate-x-1"}`} />
                </button>
              </div>
              <div className="flex items-center justify-between rounded-lg border border-[#E5E7EB] px-3 py-2.5">
                <span className="text-sm text-[#374151]">Worshipped online before?</span>
                <button type="button" onClick={() => setFromOnline(!fromOnline)}
                  ...
                  <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform ${fromOnline ? "translate-x-4" : "translate-x-1"}`} />
                </button>
              </div>
              {/* Medium of invitation */}
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151]">How did you hear about us?</label>
                <div className="relative">
              ...
              {/* Service rating */}
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151]">Service Rating (1–5)</label>
                    ...
                    <button key={r} type="button" onClick={() => setRating(r)}
                      className={`flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-medium transition-colors ${
                        rating === r
                          ...
                          : "border-[#E5E7EB] text-[#374151] hover:border-[#2563EB]"
                      }`}>
                      {r}
                    </button>
                  ))}
              ...
              {/* Favourite part */}
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs font-semibold text-[#374151]">Favourite part of service</label>
                <input value={favPart} onChange={(e) => setFavPart(e.target.value)} placeholder="e.g. Worship, Sermon, Prayer…" className={inp} />
              </div>
              {/* How was service */}
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs font-semibold text-[#374151]">How was the service overall?</label>
                <div className="relative">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            ...
            {history.length === 0 ? (
              <p className="text-xs text-[#9CA3AF]">No registrations yet.</p>
            ) : (
