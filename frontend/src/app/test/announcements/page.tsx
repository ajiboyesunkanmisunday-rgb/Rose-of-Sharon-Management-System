    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#6B7280] dark:text-slate-400">Simulates a member submitting an announcement for admin approval</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
            ...
            <h2 className="mb-1 text-base font-bold text-[#111827] dark:text-slate-100">Submit an Announcement</h2>
            <p className="mb-5 text-xs text-[#6B7280] dark:text-slate-400">Your announcement will be reviewed by the admin team before being published.</p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={subject} onChange={(e) => setSubject(e.target.value)}
                  ...
                  className="w-full rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm outline-none focus:border-sky-400" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Announcement Content *</label>
                <textarea value={content} onChange={(e) => setContent(e.target.value)}
                  placeholder="Write the full announcement text here…" rows={5}
                  className="w-full resize-none rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm outline-none focus:border-sky-400" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Display From</label>
                  <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                    className="w-full rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm outline-none focus:border-sky-400" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Display Until</label>
