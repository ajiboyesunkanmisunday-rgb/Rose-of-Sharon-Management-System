    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#6B7280] dark:text-slate-400">Simulates a member submitting a request on the public website</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
            ...
            {/* Type tabs */}
            <div className="mb-5 flex rounded-xl border border-[#E5E7EB] dark:border-slate-700 p-1 bg-[#F9FAFB]">
              {(["prayer", "counseling", "suggestion"] as RequestType[]).map((t) => (
                <button key={t} onClick={() => setType(t)}
                  className={`flex-1 rounded-lg py-2 text-xs font-semibold capitalize transition-all ${type === t ? "bg-white dark:bg-slate-800 shadow text-[#111827] dark:text-slate-100" : "text-[#6B7280] dark:text-slate-400 hover:text-[#374151] dark:text-slate-300"}`}>
                  {TYPE_CONFIG[t].label}
                </button>
              ))}
            </div>

            <h2 className="mb-1 text-base font-bold text-[#111827] dark:text-slate-100">{cfg.label}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={subject} onChange={(e) => setSubject(e.target.value)}
                  ...
                  className="w-full rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm outline-none focus:border-red-300 focus:ring-1 focus:ring-red-300" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Details *</label>
                <textarea value={content} onChange={(e) => setContent(e.target.value)}
                  placeholder={cfg.placeholder} rows={5}
                  className="w-full resize-none rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm outline-none focus:border-red-300 focus:ring-1 focus:ring-red-300" />
              </div>
            </div>
          ...
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
            ...
            <div className="mt-3 flex flex-col gap-2">
              <button onClick={() => router.push("/prayer-requests")}
                className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2 text-xs font-medium hover:bg-[#F9FAFB]">
                ...
                {history.map((h, i) => (
                  <li key={i} className={`rounded-lg border px-3 py-2 text-xs ${h.status === "ok" ? "border-green-200 bg-green-50 dark:bg-green-900/20" : "border-red-200 bg-red-50 dark:bg-red-900/20"}`}>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="ml-auto text-[#9CA3AF] dark:text-slate-400 capitalize">{h.type}</span>
                    </div>
