    ...
    <DashboardLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#6B7280]">Simulates a member announcing a celebration event on the public website</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-6">
            ...
            <h2 className="mb-1 text-base font-bold text-[#111827]">Share a Celebration</h2>
            <p className="mb-5 text-xs text-[#6B7280]">Let us celebrate this special moment with you.</p>

            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151]">Celebration Type *</label>
                <div className="relative">
                  <select value={type} onChange={(e) => setType(e.target.value)}
                    className="w-full appearance-none rounded-lg border border-[#E5E7EB] px-3 py-2.5 pr-8 text-sm text-[#111827] outline-none focus:border-purple-400">
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151]">Date *</label>
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
                  className="w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm outline-none focus:border-purple-400" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151]">Additional Notes</label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
                  placeholder="Any additional details you'd like to share…" rows={4}
                  className="w-full resize-none rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm outline-none focus:border-purple-400" />
              </div>
            </div>
                ...
                {history.map((h, i) => (
                  <li key={i} className={`rounded-lg border px-3 py-2 text-xs ${h.status === "ok" ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="ml-auto text-[#9CA3AF]">{h.date}</span>
                    </div>
