  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[24px] font-bold text-[#000000] dark:text-slate-100">Testimony Submission <span className="text-sm font-semibold text-amber-600">[TEST]</span></h1>
          <p className="text-sm text-[#6B7280] dark:text-slate-400">Simulates a member submitting a testimony on the public website</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
            ...
            <h2 className="mb-1 text-base font-bold text-[#111827] dark:text-slate-100">Share Your Testimony</h2>
            <p className="mb-5 text-xs text-[#6B7280] dark:text-slate-400">Tell us how God has been faithful in your life.</p>

            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Subject *</label>
                <input value={subject} onChange={(e) => setSubject(e.target.value)}
                  placeholder="e.g. God healed my daughter"
                  className="w-full rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm text-[#111827] dark:text-slate-100 outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#374151] dark:text-slate-300">Your Testimony *</label>
                <textarea value={content} onChange={(e) => setContent(e.target.value)}
                  placeholder="Share what God has done for you…"
                  rows={5}
                  className="w-full resize-none rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm text-[#111827] dark:text-slate-100 outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" />
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
            ...
            <button
              ...
              className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2 text-xs font-medium text-[#374151] dark:text-slate-300 hover:bg-[#F9FAFB]"
            >
          ...
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
            ...
            {history.length === 0 ? (
              <p className="text-xs text-[#9CA3AF] dark:text-slate-400">No submissions yet.</p>
            ) : (
                ...
                {history.map((h, i) => (
                  <li key={i} className={`rounded-lg border px-3 py-2 text-xs ${h.status === "ok" ? "border-green-200 bg-green-50 dark:bg-green-900/20" : "border-red-200 bg-red-50 dark:bg-red-900/20"}`}>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-semibold text-[#111827] dark:text-slate-100">{h.subject}</span>
                    </div>
