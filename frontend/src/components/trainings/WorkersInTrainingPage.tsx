      ...
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 shadow-xl">
        ...
        <div className="p-5">
          <textarea
            ...
            className="w-full rounded-lg border border-[#E5E7EB] dark:border-slate-700 px-3 py-2.5 text-sm text-[#111827] dark:text-slate-100 placeholder-[#9CA3AF] focus:border-[#7C3AED] focus:outline-none resize-none"
          />
        </div>
        <div className="flex justify-end gap-2 border-t border-[#E5E7EB] dark:border-slate-700 px-5 py-4">
          <button
  ...
  return (
    <div
      className={`relative flex flex-col rounded-xl border bg-white dark:bg-slate-800 p-5 shadow-sm transition-shadow hover:shadow-md ${
        selected ? "border-[#7C3AED] ring-1 ring-[#7C3AED]" : "border-[#E5E7EB] dark:border-slate-700"
      }`}
    >
      {/* Select checkbox */}
      <input
      ...
      {worker.occupation && (
        <p className="mt-1 text-xs text-[#6B7280] dark:text-slate-400">{worker.occupation}</p>
      )}
        ...
        <div className="mt-2 space-y-0.5">
          {worker.yourMinistry && (
            <p className="text-[10px] text-[#6B7280] dark:text-slate-400">
              <span className="font-medium text-[#374151] dark:text-slate-300">Ministry:</span>{" "}
              {worker.yourMinistry}
            </p>
          )}
          {(worker.giftsManifesting?.length ?? 0) > 0 && (
            <p className="text-[10px] text-[#6B7280] dark:text-slate-400">
              <span className="font-medium text-[#374151] dark:text-slate-300">Gifts:</span>{" "}
    ...
    <DashboardLayout>
      ...
      {!loading && (
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-center">
            <p className="text-2xl font-bold text-[#7C3AED] dark:text-purple-400">{allWorkers.length}</p>
          ...
          <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-center">
            <p className="text-2xl font-bold text-[#000080] dark:text-indigo-400">{availableSets.length}</p>
      ...
      {!loading && allWorkers.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-4 text-xs text-[#9CA3AF] dark:text-slate-400">
          ...
          <span className="flex items-center gap-1.5">
            <Star className="h-3.5 w-3.5 text-[#7C3AED] dark:text-purple-400" />
            Set badge
          </span>
        </div>
      )}
